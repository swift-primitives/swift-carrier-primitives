...
extension Carrier.`Protocol` where Self: ~Copyable & ~Escapable {
    /// Constructs a carrier from an underlying value, validating it first
    /// through the supplied closure. Throws the closure's error type when
    /// validation fails; otherwise delegates to the existing non-throwing
    /// `init(_ underlying:)` requirement.
    ///
