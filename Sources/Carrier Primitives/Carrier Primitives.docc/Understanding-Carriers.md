...
The three specificity levels compose cleanly. See `Research/capability-lift-pattern.md` for the analysis of when each fits.
...
| You're writing a type with validating init (like an enum raw-value case) | No — use `RawRepresentable` instead. See `Research/carrier-vs-rawrepresentable-comparative-analysis.md`. |
