...
At use sites: the primary associated type lands on `Underlying`, not `Domain`. That's the right call for consumers who want `some Carrier<Int>` — but it means a consumer constraining on domain (`some Carrier where Domain == UserTag`) is back to the verbose spelling. If a significant fraction of ecosystem use sites constrain on `Domain`, the PAT choice is pointing the wrong way. Do you have call-site telemetry from the existing Tagged/Cardinal/Ordinal consumers that informs this?
...
The inline DocC comments on the protocol requirements themselves are good — the comment on `Domain` explaining the `UserTag`/`OrderTag` distinction at `Sources/Carrier Primitives/Carrier.swift:62` is doing real work. I'd mirror that shape of comment onto the `underlying` property requirement and the `init(_:)` requirement.
